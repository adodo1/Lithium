    ...
    L.Control.PolylineMeasure = L.Control.extend({
        ...
        _toggleMeasure: function () {
            ...
            if (this._measuring) {   // if measuring is going to be switched on
				L.DomUtil.addClass(this._measureControl, 'active');
				...
				L.DomUtil.removeClass(this._measureControl, 'active');
        ...
        _updateTooltip: function (currentTooltip, prevTooltip, total, difference, lastCircleCoords, mouseCoords) {
            ...
            if (differenceRound.value > 0 ) {
                if (this.options.showBearings === true) {
					if (this.options.showBearingIn === true) {
						 textCurrent = this.options.bearingTextIn + ': ' + angleIn + '°<br>';
					}
                    textCurrent += this.options.bearingTextOut+':---°';
                }
                textCurrent += '<div class="polyline-measure-tooltip-difference">Длина отрезка: ' + differenceRound.value + '&nbsp;' +  differenceRound.unit + '</div>';
            }
            textCurrent += '<div class="polyline-measure-tooltip-total">Общая длина: ' + totalRound.value + '&nbsp;' +  totalRound.unit + '</div>';
	    ...
	    var polylineState = this;
            ...
            if (this.options.showBearings === true) {
				...
				text = '';
				if (this.options.showBearingIn === true) {
					 text = this.options.bearingTextIn+':---°<br>';
				}
