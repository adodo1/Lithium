(function () {
    ...
    var layerGroup,
		...
		proxy = protocol + '//pkk5.kosmosnimki.ru/',	// http://kosmosnimki.ru/pkk5/ http://pkk5.rosreestr.ru/
		// proxy = protocol + '//pkk5.kosmosnimki.ru/',	// http://kosmosnimki.ru/pkk5/ http://pkk5.rosreestr.ru/
		toLayerId = '877F789A985C416EA539B7977B404371',
		cadastreCacheLayer = {
			host: '//maps.kosmosnimki.ru',
			script: '/rest/ver1/layers/' + toLayerId + '/create'
			// ,
			// script: '/VectorLayer/ModifyVectorObjects.ashx',
			// id: '877F789A985C416EA539B7977B404371'
		},
		exportIcon,
		lastFeature,
		lastFeatureId,
	...
	L.ImageOverlay.CrossOrigin = L.ImageOverlay.extend({
	  ...
	  exportGeometry: function (feature) {
		...
		if (rings.length) {
			var obj = rings.length > 1 && L.version === '0.7.7' ? L.multiPolygon(rings) : L.polygon(rings[0]),
				opt = feature ? {cadastreFeature: feature, title: feature.attrs.cn} : null,
				geo = _map.gmxDrawing.add(obj, opt);
	...
	*/
    ...
    L.CadUtils = {
				...
				.on('load', function(ev) {
					...
					if (len && target._leaflet_id === lastOverlays[len - 1]._leaflet_id) {
						...
						if (inputShowObject && inputShowObject.checked) {
							var geo = target.exportGeometry(lastFeature);
							if (geo && lastFeature) {
								geo
								...
								// if (nsGmx && nsGmx.AuthManager && nsGmx.AuthManager.isLogin()) { L.CadUtils.addToCadastreLayer(geo.toGeoJSON()); }
							}
						}
					}
                });
        ...
        addToCadastreLayer: function(obj) {
			var feature = obj.properties.cadastreFeature;
			
			L.gmx.getJSON(cadastreCacheLayer.host + cadastreCacheLayer.script, {
				options: {
					type: 'json'
				},
				params: {
					...
					feature: JSON.stringify({
						properties: {
							cn: feature.attrs.cn,
							address: feature.attrs.address,
							center: JSON.stringify(feature.center),
							extent: JSON.stringify(feature.extent),
							date: parseInt(Date.now()/1000),
							attrs: JSON.stringify(feature.attrs),
							user: nsGmx && nsGmx.AuthManager ? nsGmx.AuthManager.getLogin() : ''
						},
...
/*
			L.gmxUtil.layerHelper.modifyObject(cadastreCacheLayer.host + cadastreCacheLayer.script, cadastreCacheLayer.id,
				[{
					properties: {
						cn: feature.attrs.cn,
						address: feature.attrs.address,
					...
					addLayer = function() {
						...
						if (!layerWMS) {
							L.gmx.loadLayers(layers).then(function(cadastreLayer, zones, addon) {
								layerWMS = L.Util.isArray(cadastreLayer) ? cadastreLayer[0] : cadastreLayer;
								// map.options.maxZoom = 22;
								//layerWMS.options.maxZoom = 22;
								layerWMS.options.zIndex = 1000000;
