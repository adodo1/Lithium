...
CloneMiniDialog.prototype.showProductYearsList = function (prod, year) {
    if (document.getElementById("yearSelClone_" + prod)) {
        this._currProd = prod;
        for (var l in this.graphDialog.pluginParams.products) {
            if (this.graphDialog.pluginParams.products[l].ndviSource) {
...
CloneMiniDialog.prototype.setProductYear = function (prod, year) {
    if (document.getElementById("yearSelClone_" + prod)) {
        var years = this.graphDialog.pluginParams.products[prod].years;
        year = parseInt(year);
